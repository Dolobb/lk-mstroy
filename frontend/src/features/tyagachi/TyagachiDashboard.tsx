...
const TyagachiDashboard: React.FC = () => {
  ...
  // Fetch lastSyncAt on mount and after each sync
  useEffect(() => {
    getSyncStatus()
  ...
  // Load vehicles when days or syncVersion changes
  useEffect(() => {
  ...
  // VehicleOverview calls this when a vehicle is expanded
  ...
  // Map TyagachiVehicle[] → RealVehicleItem[]
  ...
  // Summary for SyncPanel (derived from loaded vehicles)
  const summary: import('./types').DashboardSummary | null = fullVehicles.length > 0
    ? {
