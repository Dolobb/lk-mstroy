...
2. Выбрать период (по умолчанию — с начала текущего месяца по сегодня)
3. Заявки сгруппированы по городам, внутри — «Активные» и «Выполненные»
4. Каждая карточка показывает: маршрут, прогресс (факт/план рейсов %), кол-во ТС, ПЛ, груз, тоннаж
5. Нажать на карточку → раскроется таблица Ганта (самосвалы × дата × смена, кол-во рейсов)
