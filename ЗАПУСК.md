...
TIS_API_TOKENS=token1,token2,token3
SERVER_PORT=3001
...
## Пересборка TypeScript (geo-admin)

geo-admin/client — единственный компонент с **ручной** пересборкой.
Его TypeScript компилируется в `dist/`, который не хранится в git.

### Когда нужна пересборка

Если менялись любые файлы в `geo-admin/client/src/`:
```powershell
cd geo-admin\client && npx tsc
```

### Остальные компоненты — пересборка НЕ нужна

| Компонент        | Режим                       | Изменения применяются    |
|------------------|-----------------------------|--------------------------|
| kip/server       | tsx watch (dev:server)      | Автоматически            |
| kip/client       | Vite dev (dev:client)       | Автоматически (HMR)      |
| dump-trucks      | tsx watch (npm run dev)     | Автоматически            |
| geo-admin/server | tsx watch (npm run dev)     | Автоматически            |
| geo-admin/client | tsc (ручная сборка)         | **Нужен npx tsc**        |
| frontend         | Vite dev (npm run dev)      | Автоматически (HMR)      |
| tyagachi         | Python uvicorn reload       | Автоматически            |

> После `npx tsc` перезапускать geo-admin/server не нужно —
> он раздаёт статику из `client/dist/` как файлы.
