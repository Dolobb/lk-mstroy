...
describe("admin routes", () => {
  ...
  afterAll(async () => {
    ...
    await db.delete(fuelReceiptEvents).where(inArray(fuelReceiptEvents.id, [receiptId]));
    await db
      .delete(fuelDispenseEvents)
      .where(inArray(fuelDispenseEvents.id, [activeDispenseId, deletedDispenseId]));
    await db.delete(shifts).where(inArray(shifts.id, [shiftId]));
    await db.delete(vehicles).where(inArray(vehicles.id, [vehicleId]));
    await db.delete(atz).where(inArray(atz.id, [atzId]));
    await db.delete(organizations).where(inArray(organizations.id, [organizationId]));
    await db.delete(drivers).where(inArray(drivers.id, [driverId]));
    await pool.end();
  });
